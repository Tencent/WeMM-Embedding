...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for query, video_id, options, answer, question_id, domain, sub_category in (
            zip(batch_dict['question'], batch_dict['videoID'], batch_dict['options'], batch_dict['answer'],
                batch_dict['question_id'], batch_dict['domain'], batch_dict['sub_category'])):
        question = query
        query = (
            f'{VLM_VIDEO_TOKENS[model_backbone]} Question:'
            f' {question}\n') + '\n'.join(options) + f'\nQuestion: {question}'
